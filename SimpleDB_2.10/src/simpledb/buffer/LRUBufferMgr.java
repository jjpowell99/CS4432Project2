...
class LRUBufferMgr extends AbsBufferMgr {
	// CS4432-Project1: Adding hashmap to keep track of blocks existing in
	// bufferpool and
	...
	/**
	 * CS4432-Project1: Added initialization of an empty hashmap and queue filled
	 * with the empty buffers.
	 * 
	...
	/**
	 * CS4432-Project1: Added a put statement to the hashmap to keep track of the
	 * buffer for blk, now that it is pinned in the buffer pool for efficient access
	 * thereafter.
	 * 
	...
	/**
	 * CS4432-Project1: Adds this buffer to the end of the queue now that it is
	 * unpinned, and thus, available to be replaced.
	 * 
	...
	/**
	 * CS4432-Project1: Changed the scan for an existing buffer to using the hashmap
	 * to find it efficiently if it exists. Additionally, the queue is updated so
	 * that this most recently accessed block would exist at the end if unpinned.
	 * Finds buffer that this block exists in if it does exist and returns it.
	 * Also added timing to test time needed to find existing block in buffer pool.
	 * 
	 * @param blk Block to find in bufferpool if it exists.
	 * @return Buffer in bufferpool that block exists in. Null is returned if the
	 *         block does not exist in the buffer pool.
	 */
	...
	/**
	 * CS4432-Project1: Changed the scan for an available buffer to instead use an
	 * LRU replacement policy. Finds the least recently used unpinned buffer and
	 * returns it. Also added timing to test time needed to choose and available buffer.
	 * 
	 * @return Available unpinned buffer that was the least recently used, or null
	 *         if all buffers are pinned
	 */
	private Buffer chooseUnpinnedBuffer() {
		long startTime = System.nanoTime();
		long endTime;
		// CS4432-Project1: While there are potential available buffers, find the first
		...
		while (possibleBuffers.size() > 0) {
			...
			} else { // CS4432-Project1: If unpinned buffer return the buffer and update the hashmap
						// if it has a block currently.
	...
	/**
	 * CS4432-Project1: Changed the toString by overwriting it to list the toStrings
	 * for each buffer in the bufferpool
	 * 
	 * @return String of toStrings of buffers in bufferpool with header describing
	 *         buffer manager type.
	 */
